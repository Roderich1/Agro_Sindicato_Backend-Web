...
- **NOT_FOUND_IN_AUDITED_REVISION:** no se encontró consumo Flutter en la revisión `fd5ce96c1b4c62285b54b038e09b13df5d5e79ee`; debe reauditarse el HEAD usado para cutover/removal.
...
- **Repositorio/revisión:** `Roderich1/AppMovilAgroquimico` @ `fd5ce96c1b4c62285b54b038e09b13df5d5e79ee`.
- **Ámbito:** `pubspec.yaml`, `pubspec.lock`, `lib/**/*.dart`, `test/**/*.dart` e `integration_test/**/*.dart` cuando existe.
- **Búsquedas:** `http`, `Dio`, `retrofit`, `baseUrl/base_url`, URLs HTTP(S), API/REST client, socket/WebSocket y rutas `/sync`, `/inventory`, `/applications`, `/plots`, `/purchases`.
- **Evidencia:** `pubspec.yaml` no declara cliente HTTP/Dio/Retrofit/WebSocket directo; `lib/` no contiene imports o construcción de clientes/red ni rutas backend encontradas. `pubspec.lock` contiene paquetes transitivos `http` y `web_socket`, lo que por sí solo no evidencia consumo de la API.
- **Resultado:** `NOT_FOUND_IN_AUDITED_REVISION`.
