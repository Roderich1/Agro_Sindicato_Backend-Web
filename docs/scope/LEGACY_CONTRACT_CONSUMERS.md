...
El inventario de modelos y consumidores es evidencia de compatibilidad, no una lista de capacidades requeridas. `Purchase` individual tiene target Mobile; `PayableAccount`/`Payment` y `CalendarEvent` no tienen target Live aprobado.
