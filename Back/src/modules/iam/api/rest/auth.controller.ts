...
export class AuthController {
  ...
  async login(
    ...
    this.logger.log(
      `[AUTH LOGIN TRY] email=${dto.email} ${requestSource(req)}`,
    );
    ...
    try {
      ...
      this.logger.log(
        `[AUTH LOGIN OK] userId=${result.user.id} email=${result.user.email} role=${result.user.role} tenantId=${result.user.tenantId} ${requestSource(req)}`,
      );

      return { accessToken: result.accessToken, user: result.user };
    } catch (error) {
      this.logger.warn(
        `[AUTH LOGIN FAIL] email=${dto.email} ${requestSource(req)} reason="${error instanceof Error ? error.message : "unknown"}"`,
      );
      throw error;
    }
  }
  ...
  async refresh(
    ...
    try {
      ...
      this.logger.log(
        `[AUTH REFRESH OK] userId=${result.user.id} email=${result.user.email} ${requestSource(req)}`,
      );

      return { accessToken: result.accessToken, user: result.user };
    } catch (error) {
      this.logger.warn(
        `[AUTH REFRESH FAIL] hasCookie=${Boolean(rawToken)} ${requestSource(req)} reason="${error instanceof Error ? error.message : "unknown"}"`,
      );
      throw error;
    }
  }
  ...
  async logout(@Req() req: Request, @Res({ passthrough: true }) res: Response) {
    ...
    this.logger.log(
      `[AUTH LOGOUT] hasCookie=${Boolean(rawToken)} ${requestSource(req)}`,
    );
  }
